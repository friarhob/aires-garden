...
- [ ] 4.1 Add `main` centred-column rule: `max-width: var(--content-width)`, `margin: 0 auto`, padding

## 5. Base template — fonts

- [ ] 5.1 Add `<link rel="preconnect" href="https://fonts.googleapis.com">` to `base.html` `<head>`
- [ ] 5.2 Add `<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>` to `base.html` `<head>`
- [ ] 5.3 Add Google Fonts stylesheet `<link>` loading Fraunces (variable, opsz + wght axes, italic + upright) and IBM Plex Sans (300, 400, 500; upright and italic)
- [ ] 5.4 Add `<link rel="stylesheet" href="/theme/css/styles.css">` to `base.html` `<head>` (if not already present)

## 6. Verification

- [ ] 6.1 Run `pelican content -s pelicanconf.py` and confirm build completes without errors
- [ ] 6.2 Open a rendered page in the browser and confirm dark-mode palette applies by default
- [ ] 6.3 Add `data-theme="light"` to `<html>` in a built page temporarily and confirm light palette switches correctly, then revert
- [ ] 6.4 Confirm headings use Fraunces and body text uses IBM Plex Sans (or readable fallbacks if offline)
- [ ] 6.5 Confirm `--text-muted` is not used at font sizes below 14px in any existing template
