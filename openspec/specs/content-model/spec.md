...
Define and enforce the frontmatter schema for all Markdown content in the garden.
Every post under `content/posts/` and every page under `content/pages/` must declare
fields that conform to their respective schemas, validated at both build time and via a
standalone CLI.  The filename-to-frontmatter contract for posts, the `Status` enum, and
the `Lang` encoding rules are all checked invariants rather than prose conventions.
