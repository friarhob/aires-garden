...
Automate the path from `git push` on `main` to a live URL at `https://garden.fernandoaires.org`. This capability owns every artifact whose existence is motivated by "the site is deployed" (not "the site is built") — the GitHub Actions workflow, the GitHub Pages custom-domain wiring, the DNS record, the pre-launch noindex layers, and the custom 404 page. Per ADR-0003, the deploy targets the pre-launch subdomain and carries a two-layer noindex posture until a future `launch-to-apex` change cuts over to the apex.
